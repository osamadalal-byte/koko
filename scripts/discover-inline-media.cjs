...
(async()=>{
 ...
 try{for(const slug of paths){
  const row={slug,url:slug.startsWith('https:')?slug:'https://www.hingehealth.com/resources/articles/'+slug+'/',requests:[]};all.push(row);
  const page=await browser.newPage({viewport:{width:900,height:800}});page.setDefaultTimeout(3000);
  ...
  try{
   ...
   if(response?.ok()){
    ...
    row.native=await videos.evaluateAll(els=>els.map(v=>({src:v.currentSrc||v.src,sources:[...v.querySelectorAll('source')].map(s=>s.src)})));
